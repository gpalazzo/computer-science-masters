...
number = -1
fhand = open("exercise4_v2.txt")  # TWO FILES: cpm.txt and cpm1.txt
            ...
            for dipendenza in tasks[k][
                ...
                if (
                    ...
                    if int(tasks["task" + dipendenza]["EF"]) > int(tasks[k]["ES"]):
                        tasks[k]["ES"] = int(tasks["task" + dipendenza]["EF"]) + 1
                        tasks[k]["EF"] = (
                            int(tasks[k]["ES"]) + int(tasks[k]["duration"]) - 1
                        )
...
for taskBW in bList:
    ...
    for dipendenza in tasks[taskBW][
        "dependencies"
    ]:  # slides all the dependency in a single task
        if dipendenza != "-1":  # check if it's NOT the last task
            if (
                ...
                tasks["task" + dipendenza]["LS"] = (
                    ...
                    - int(tasks["task" + dipendenza]["duration"])
                    + 1
                )
